...
class LyapunovCalculator:
    ...
    def _rosenstein_method(
        self, time_series: np.ndarray, dt: float, min_tsep: Optional[int]
    ) -> float:
        ...
        for i in range(n_points - max_iter):
            ...
            for k in range(max_iter):
                ...
                current_dist = np.linalg.norm(embedded[i + k] - embedded[nn_idx + k])
        ...
        slope, _, _, _, _ = stats.linregress(
            time_steps[valid_points], log_divergence[valid_points]
        )
    ...
    def _time_delay_embedding(
        self, time_series: np.ndarray, tau: int = 1
    ) -> np.ndarray:
        """
...
class PhaseSpaceAnalyzer:
    ...
    def calculate_recurrence_plot(
        self, trajectory: np.ndarray, threshold: float = 0.1
    ) -> np.ndarray:
        """
    ...
    def poincare_section(
        ...
        return (
            np.array(crossings)
            if crossings
            else np.array([]).reshape(0, trajectory.shape[1])
        )
...
class StatisticalAnalyzer:
    ...
    def autocorrelation_test(
        self, sequence: np.ndarray, max_lag: int = 100
    ...
    def entropy_measures(
        self, sequence: np.ndarray, block_size: int = 8
    ) -> dict[str, float]:
        """
