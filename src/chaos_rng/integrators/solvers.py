...
class AdaptiveSolver(ODESolver):
    ...
    def integrate(
        ...
        return _adaptive_rk45_jit(
            f, y0.astype(np.float64), 
            t_span[0], t_span[1],
            self.rtol, self.atol, max_step
        )


@numba.njit(cache=True)
def _rk45_step(f_func, t: float, y: np.ndarray, h: float) -> Tuple[np.ndarray, np.ndarray, float]:
    """
    ...
    k6 = h * f_func(t + a6*h, y + b61*k1 + b62*k2 + b63*k3 + b64*k4 + b65*k5)
...
def _adaptive_rk45_jit(
    f_func, y0: np.ndarray, t0: float, tf: float,
    rtol: float, atol: float, max_step: Optional[float]
) -> Tuple[np.ndarray, np.ndarray]:
    """
