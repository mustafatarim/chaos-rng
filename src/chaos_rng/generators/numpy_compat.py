...
try:
    ...
    NUMPY_INTERFACE_AVAILABLE = True
except ImportError:
    NUMPY_INTERFACE_AVAILABLE = False
...
class ChaosBitGenerator(BitGenerator):
    ...
    def random_raw(self, size: Optional[int] = None, dtype: np.dtype = np.uint64) -> Union[int, np.ndarray]:
        """
    ...
    def random(self, size: Optional[Union[int, Tuple[int, ...]]] = None, dtype: np.dtype = np.float64) -> Union[float, np.ndarray]:
        """
